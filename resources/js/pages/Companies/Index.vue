...
import { Actions, EntityKey, Company, CompanyActions } from './dtos/data';
import CompanyCard from './forms/CompanyCard.vue';
...
const editingRecord = ref<Company | null>(null);
...
const openEditDialog = (record: Company) => {
...
const fetchData = async (page: number, perPage: number) => {
    const response = await axios.get<PaginationResponse<Company>>(route('companies.index'), { params: { page, per_page: perPage } });
...
const handleEditRecord = (record: Company) => {
    openEditDialog(record);
};

const handleDeleteRecord = (record: Company) => {
    deleteRecord(record);
};
...
onMounted(() => {
    ...
    window.addEventListener(getActionEventName(CompanyActions.edit, EntityKey), handleEditEvent);
    window.addEventListener(getActionEventName(CompanyActions.delete, EntityKey), handleDeleteEvent);
...
onUnmounted(() => {
    window.removeEventListener(getActionEventName(CompanyActions.edit, EntityKey), (window as any).__editRecordHandler);
    window.removeEventListener(getActionEventName(CompanyActions.delete, EntityKey), (window as any).__deleteRecordHandler);
});

const deleteRecord = async (record: Company) => {
    if (!confirm(`Are you sure you want to delete "${record.name}"?`)) return;

    try {
        await router.delete(`/companies/${record.id}`);
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-3">
            <PageHeader title="Companies" description="Manage your companies" show-view-toggle
            ...
            <div v-else-if="currentView === 'grid'" class="flex-[100%]">
                <CrudGrid ref="crudGridRef" :data-source="fetchData" :card-component="CompanyCard"
                    item-prop-name="record" empty-message="No companies yet" :empty-icon="Dumbbell"
                    empty-action-text="Create Your First Company" :empty-action="openCreateDialog"
            ...
            <Dialog v-model:open="isEditDialogOpen">
                <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
                    ...
                    <CreateCompanyForm v-if="editingRecord" :record="editingRecord" :on-success="handleSaveSuccess"
            ...
            <Dialog v-model:open="isCreateDialogOpen">
                <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
                    ...
                    <CreateCompanyForm :record="editingRecord" :on-success="handleSaveSuccess"
                        :on-cancel="closeDialogs" />
                </DialogContent>
            </Dialog>
        </div>
