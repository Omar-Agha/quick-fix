...
import { EntityKey, Article } from '../dtos/data';
import { EntityAction } from '@/components/dv-components/common';


interface Props {
    record: Article;
    entityActions?: EntityAction<Article>[];
}
    ...
    <Card class="transition-shadow hover:shadow-lg">
        <CardHeader>
            <div class="flex items-start justify-between">
                <div class="space-y-1">
                    <CardTitle class="text-lg">{{ record.title }}</CardTitle>
                    <CardDescription v-if="record.content">
                        <!-- {{ record.content }} -->
                    </CardDescription>
                </div>
                <DropdownMenu>
            ...
            <div class="space-y-3">
                ...
                <div class="flex flex-col md:flex-row gap-4 items-center flex-wrap">
                    ...
                    <div class="flex-1 space-y-1 w-full">
                        <div v-if="record.content" class="text-gray-600 dark:text-gray-300">
                            {{ record.content }}
                        </div>
