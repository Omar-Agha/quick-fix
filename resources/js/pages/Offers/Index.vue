...
import { Actions, EntityKey, Offer, OfferActions } from './dtos/data';
import OfferCard from './forms/OfferCard.vue';
import { columns } from './columns';

import { getActionEventName } from '@/components/dv-components/common'
import CreateOfferForm from './forms/CreateOfferForm.vue';
...
const editingRecord = ref<Offer | null>(null);
...
const openEditDialog = (record: Offer) => {
...
const fetchData = async (page: number, perPage: number) => {
    const response = await axios.get<PaginationResponse<Offer>>(route('offers.index'), { params: { page, per_page: perPage } });
...
const handleEditRecord = (record: Offer) => {
    openEditDialog(record);
};

const handleDeleteRecord = (record: Offer) => {
    deleteRecord(record);
};
...
onMounted(() => {
    ...
    window.addEventListener(getActionEventName(OfferActions.edit, EntityKey), handleEditEvent);
    window.addEventListener(getActionEventName(OfferActions.delete, EntityKey), handleDeleteEvent);
...
onUnmounted(() => {
    window.removeEventListener(getActionEventName(OfferActions.edit, EntityKey), (window as any).__editRecordHandler);
    window.removeEventListener(getActionEventName(OfferActions.delete, EntityKey), (window as any).__deleteRecordHandler);
});

const deleteRecord = async (record: Offer) => {
    if (!confirm(`Are you sure you want to delete "${record.name}"?`)) return;

    try {
        await router.delete(`/offers/${record.id}`);
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-3">
            <PageHeader title="Offers" description="Manage your offers" show-view-toggle
            ...
            <div v-else-if="currentView === 'grid'" class="flex-[100%]">
                <CrudGrid ref="crudGridRef" :data-source="fetchData" :card-component="OfferCard" item-prop-name="record"
                    empty-message="No offers yet" :empty-icon="Image" empty-action-text="Create Your First Offer"
            ...
            <Dialog v-model:open="isEditDialogOpen">
                <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
                    ...
                    <CreateOfferForm :record="editingRecord" :on-success="handleSaveSuccess"
